...
export class ImportadorService {
  ...
  private cleanDoctorName(name: string): string {
    ...
    // Quitar prefijos Dr. Dra. Dr o Dra
  ...
  async importarExcel(fileBuffer: Buffer, dryRun: boolean) {
    ...
    if (consultasSheet) {
      ...
      for (let i = 4; i < rows.length; i++) {
        ...
        const especialidad = this.standardizeSpecialty(row[6] ? String(row[6]) : '');
    ...
    if (ecografiasSheet) {
      ...
      for (let i = 4; i < rows.length; i++) {
        ...
        const grado = row[4] ? String(row[4]).trim() : 'Doctor';
        const medicoName = this.cleanDoctorName(String(row[5] || ''));
        const especialidad = this.standardizeSpecialty(row[6] ? String(row[6]) : 'Ginecología');
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const tk of ticketsAImportar) {
        ...
        cat = this.standardizeCategory(cat);
        const stdEsp = this.standardizeSpecialty(tk.especialidad);
