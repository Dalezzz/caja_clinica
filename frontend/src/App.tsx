...
function App() {
  ...
  const [procedencias, setProcedencias] =
    useState<Procedencia[]>(INITIAL_PROCEDENCIAS);
  ...
  const [medicoSolicitanteId, setMedicoSolicitanteId] = useState<
    number | undefined
  >(undefined);
  ...
  const handleTriggerAdminMode = async () => {
    if (currentUser?.rol === "ADMINISTRADOR") {
      // Si es admin, degradar automáticamente a recepcionista
      setAdminModalLoading(true);
      try {
        const response = await api.login<any>("user", "user1234");
        setAuthToken(response.access_token);
        localStorage.setItem("currentUser", JSON.stringify(response.usuario));
        setCurrentUser(response.usuario);
        setActiveTab("pos");
      } catch (err: any) {
        alert("Error al retornar a modo recepcionista: " + err.message);
      } finally {
        setAdminModalLoading(false);
      }
    } else {
      // Mostrar modal para ingresar la contraseña de admin
      setAdminPasswordInput("");
      setAdminModalError(null);
      setShowAdminModeModal(true);
    }
  };

  const handleAdminModeSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!adminPasswordInput) return;
    setAdminModalError(null);
    setAdminModalLoading(true);
    try {
      const response = await api.login<any>("admin", adminPasswordInput);
      setAuthToken(response.access_token);
      localStorage.setItem("currentUser", JSON.stringify(response.usuario));
      setCurrentUser(response.usuario);
      setShowAdminModeModal(false);
      setActiveTab("admin");
    } catch (err: any) {
      setAdminModalError("Contraseña de administrador incorrecta");
    } finally {
      setAdminModalLoading(false);
    }
  };
  ...
  const [newMedicoErrors, setNewMedicoErrors] = useState<
    Partial<Record<keyof Medico, string>>
  >({});
  const [showAddTarifa, setShowAddTarifa] = useState(false);
  const [newTarifaData, setNewTarifaData] = useState<Partial<Tarifa>>({
    requiereTecnico: false,
  });
  const [newTarifaErrors, setNewTarifaErrors] = useState<
    Partial<Record<keyof Tarifa, string>>
  >({});
  const [showAddProc, setShowAddProc] = useState(false);
  const [newProcData, setNewProcData] = useState<Partial<Procedencia>>({});
  const [newProcErrors, setNewProcErrors] = useState<
    Partial<Record<keyof Procedencia, string>>
  >({});
  ...
  const handleAnularTicket = async (ticketId: number) => {
    if (
      !window.confirm(
        "¿Está seguro de anular este comprobante? Esta acción registrará el ticket como ANULADO.",
      )
    )
      return;
    try {
      ...
      setTickets((prev) =>
        prev.map((t) => (t.id === ticketId ? { ...t, estado: "ANULADO" } : t)),
      );
  ...
  const handleSaveMedico = async (id: number) => {
    ...
    setAdminError(null);
    try {
      const updated = await api.patch<Medico>(
        `medicos/${id}`,
        editingMedicoData,
      );
      setMedicos((prev) =>
      ...
      setAdminError(
        "No se pudo guardar. Verifica la conexión con el servidor.",
      );
    } finally {
  ...
  const handleDeleteMedico = async (id: number) => {
    ...
    try {
      ...
      setAdminError(
        "No se puede eliminar: el médico tiene atenciones registradas en el sistema.",
      );
    } finally {
      setAdminSaving(false);
    }
  ...
  const handleSaveTarifa = async (id: number) => {
    ...
    setAdminError(null);
    try {
      const updated = await api.patch<Tarifa>(
        `tarifas/${id}`,
        editingTarifaData,
      );
      setTarifas((prev) =>
      ...
      setAdminError("No se pudo guardar la tarifa.");
    } finally {
      setAdminSaving(false);
    }
  };
  ...
  const handleSaveProc = async (id: number) => {
    ...
    setAdminError(null);
    try {
      const updated = await api.patch<Procedencia>(
        `procedencias/${id}`,
        editingProcData,
      );
      ...
      setAdminError("No se pudo guardar la procedencia.");
    } finally {
      setAdminSaving(false);
    }
  };
  ...
  const handleDeleteProc = async (id: number) => {
    if (
      !window.confirm(
        "¿Eliminar esta procedencia? Solo es posible si no tiene pacientes asociados.",
      )
    )
      return;
    setAdminSaving(true);
    ...
    try {
      ...
      setAdminError(
        "No se puede eliminar: la procedencia tiene pacientes registrados.",
      );
    } finally {
      setAdminSaving(false);
    }
  ...
  const handleCreateMedico = async () => {
    ...
    if (!newMedicoData.nombre || newMedicoData.nombre.trim() === "")
      errors.nombre = "Nombre es requerido";
    if (!newMedicoData.especialidad || newMedicoData.especialidad.trim() === "")
      errors.especialidad = "Especialidad es requerida";
    if (!newMedicoData.grado || newMedicoData.grado.trim() === "")
  ...
  const handleCreateTarifa = async () => {
    ...
    if (
      !newTarifaData.categoria ||
      String(newTarifaData.categoria).trim() === ""
    )
      errors.categoria = "Categoría es requerida";
    if (
      !newTarifaData.descripcion ||
      String(newTarifaData.descripcion).trim() === ""
    )
      errors.descripcion = "Descripción es requerida";
    if (
      newTarifaData.precioTotal == null ||
      Number.isNaN(Number(newTarifaData.precioTotal))
    )
      errors.precioTotal = "Precio válido es requerido";
    if (
      newTarifaData.comisionMedico == null ||
      Number.isNaN(Number(newTarifaData.comisionMedico))
    )
      errors.comisionMedico = "Comisión médico válida es requerida";
    if (
      newTarifaData.comisionClinica == null ||
      Number.isNaN(Number(newTarifaData.comisionClinica))
    )
      errors.comisionClinica = "Comisión clínica válida es requerida";
  ...
  const handleCreateProc = async () => {
    ...
    if (!newProcData.nombre || String(newProcData.nombre).trim() === "")
      errors.nombre = "Nombre es requerido";
  ...
  const handleDryRun = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/importar/excel?dryRun=true`,
        {
          method: "POST",
          body: formData,
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      formData.append("file", excelFile);
      const response = await fetch(
        `${API_BASE_URL}/importar/excel?dryRun=false`,
        {
          method: "POST",
          body: formData,
          headers: buildHeaders(),
      ...
      if (!response.ok) {
        const err = await response.json();
        throw new Error(
          err.message || "Error al guardar los datos en base de datos.",
        );
      }
  ...
  const handleSaveAjustes = async (
    ...
    try {
      ...
      setAdminError(
        `No se pudo guardar la configuración: ${err.message || "Error de conexión"}`,
      );
    } finally {
      setAdminSaving(false);
    }
  ...
  const handleOpenCaja = async () => {
    try {
      const newCaja = await api.post<CajaDiaria>("cajas-diarias", {
        montoApertura: 100,
      });
      setCaja(newCaja);
      ...
      alert(
        `No se pudo abrir la caja en el servidor. Asegúrate de que el backend esté ejecutándose.`,
      );
    }
  };
  ...
  const handleCloseCaja = async () => {
    ...
    try {
      ...
      alert(
        "Caja Diaria Cerrada y Reporte de Auditoría Generado en el servidor.",
      );
    } catch (err: any) {
  ...
  const handleDocOrPhoneSearch = (val: string, field: "doc" | "phone") => {
    ...
    if (term.length >= 6) {
      ...
      if (match && match.paciente) {
        ...
        if (match.paciente.numeroDocumento)
          setNumDoc(match.paciente.numeroDocumento);
    ...
    setCartItems((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          comisionTecnico: targetTarifa.requiereTecnico
            ? Number(targetTarifa.comisionTecnico)
            : 0,
        },
      ];
    });
  ...
  // Cart Calculations
  const totalMontoPaciente = cartItems.reduce(
    (sum, i) => sum + i.precioUnitario * i.cantidad,
    0,
  );
  const totalMontoMedico = cartItems.reduce(
    (sum, i) => sum + i.comisionMedico * i.cantidad,
    0,
  );
  const totalMontoTecnico = cartItems.reduce(
    (sum, i) => sum + i.comisionTecnico * i.cantidad,
    0,
  );
  const totalMontoSolicitante = medicoSolicitanteId ? 20 : 0;
  const totalMontoClinica = Math.max(
    0,
    totalMontoPaciente -
      totalMontoMedico -
      totalMontoTecnico -
      totalMontoSolicitante,
  );
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    if (!caja) {
      alert(
        "No hay caja diaria abierta. Abre la caja antes de registrar comprobantes.",
      );
      return;
    }
    ...
    // Construir la lista efectiva de items — si el carrito está vacío, usar la tarifa del dropdown
    let effectiveItems: TicketItem[] = cartItems;
    if (cartItems.length === 0) {
      const targetTarifa = tarifas.find((t) => t.id === tarifaSeleccionadaId);
      if (!targetTarifa) {
        alert("Selecciona un servicio médico para agregar al comprobante.");
        return;
      }
      effectiveItems = [
        {
      ...
      // Actualizar también el estado visual del carrito
      setCartItems(effectiveItems);
    }
    ...
    const procedenciaSelected =
      procedencias.find((p) => p.id === procedenciaId) || procedencias[0];
    ...
    try {
      ...
      const savedTicket = await api.post<Ticket>("tickets", {
        ...
        tarifaId: effectiveItems[0]?.tarifaId,
        ...
        metodoPago,
        descripcionAdicional: effectiveItems
          .map((i) => i.descripcion)
          .join(", "),
      });

      // PASO 3: Construir el objeto local solo con datos puramente de vista
        ...
        numeroBoleta:
          tipoComprobante === "BOLETA_ELECTRONICA"
            ...
            : tipoComprobante === "FACTURA_ELECTRONICA"
              ...
              : undefined,
        rucFactura:
          tipoComprobante === "FACTURA_ELECTRONICA" ? rucFactura : undefined,
        razonSocialFactura:
          tipoComprobante === "FACTURA_ELECTRONICA"
            ? razonSocialFactura
            : undefined,
      ...
      setPrintedTicket(finalTicket);
      resetForm();
    } catch (err: any) {
      console.error("Error al registrar atención:", err);
      alert(
        `Error al guardar en la base de datos: ${err?.message || "Verifica que el servidor esté activo."}`,
      );
    }
  };

  const handleUpdateEstadoAtencion = async (
    ticketId: number,
    nextEstado: EstadoAtencion,
  ) => {
    // Optimistic UI update for immediate feedback
    const previousTickets = [...tickets];
    setTickets((prev) =>
      prev.map((t) =>
        t.id === ticketId ? { ...t, estadoAtencion: nextEstado } : t,
      ),
    );

    try {
      await api.actualizarEstadoTicket(ticketId, nextEstado);
    } catch (err: any) {
      console.error("Error al actualizar el estado:", err);
      alert("Error al sincronizar el estado con el servidor.");
      // Revert if API fails
  ...
  const handleCreateEgreso = async (e: React.FormEvent) => {
    ...
    if (caja) {
      setCaja({
        ...caja,
        montoEfectivoEsperado:
          Number(caja.montoEfectivoEsperado) - Number(montoEgreso),
      });
    }
    setMontoEgreso("");
    ...
    <div className="min-h-screen bg-[#fafafa] text-zinc-950 flex flex-row font-sans selection:bg-zinc-900 selection:text-white">
      <AppSidebar
        ...
        ticketCount={
          tickets.filter(
            (t) =>
              caja &&
              t.cajaDiariaId === caja.id &&
              t.estadoAtencion !== "ATENDIDO",
          ).length
        }
        userRole={currentUser.rol}
        userName={currentUser.nombre}
        onTriggerAdminMode={handleTriggerAdminMode}
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        ...
        <main className="w-full p-6 flex-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-7 space-y-5">
                <form
                  ...
                  className="white-card rounded-lg p-6 shadow-sm border border-zinc-200 space-y-5"
                >
                  <div className="flex items-center justify-between border-b border-zinc-100 pb-3">
                    <div className="flex items-center gap-2">
                      <UserPlus className="h-4 w-4 text-zinc-900" />
                      <h2 className="text-sm font-semibold text-zinc-900">
                        Registrar Atención de Paciente
                      </h2>
                    </div>
                  ...
                  <div className="bg-zinc-50/50 rounded-lg p-4 border border-zinc-200/80 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-semibold text-zinc-700 uppercase tracking-wider flex items-center gap-1.5">
                        <UserCheck className="h-3.5 w-3.5" /> 1. Datos Clínicos
                        del Paciente
                      </span>
                      {isExistingPatient && (
                        <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-medium px-2 py-0.5 rounded-md flex items-center gap-1">
                          <CheckCircle2 className="h-3 w-3" /> Paciente
                          Registrado
                        </span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                      ...
                      <div className="sm:col-span-4">
                        ...
                        <div className="flex gap-1">
                          ...
                          <input
                            ...
                            value={numDoc}
                            onChange={(e) =>
                              handleDocOrPhoneSearch(e.target.value, "doc")
                            }
                      ...
                      <div className="sm:col-span-4">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleDocOrPhoneSearch(e.target.value, "phone")
                          }
                      ...
                      <div className="sm:col-span-4">
                        ...
                        <select
                          value={procedenciaId}
                          onChange={(e) =>
                            setProcedenciaId(Number(e.target.value))
                          }
                  ...
                  <div className="bg-zinc-50/50 rounded-lg p-4 border border-zinc-200/80 space-y-3">
                    <span className="text-[11px] font-semibold text-zinc-700 uppercase tracking-wider flex items-center gap-1.5">
                      <Stethoscope className="h-3.5 w-3.5" /> 2. Asignación
                      Médica CMP y Servicios
                    </span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="block text-[11px] font-medium text-zinc-500 mb-1">
                          Médico Tratante (CMP / Consultorio) *
                        </label>
                        <select
                          value={medicoId}
                        ...
                        >
                          {medicos.map((m) => (
                            <option key={m.id} value={m.id}>
                              {m.grado} {m.nombre} — CMP: {m.cmp || "S/N"} (
                              {m.consultorioAsignado})
                            </option>
                          ))}
                        </select>
                      </div>
                    ...
                    <div className="pt-1">
                      <label className="block text-[11px] font-medium text-zinc-500 mb-1">
                        Añadir Servicios al Comprobante
                      </label>
                      <div className="flex gap-2">
                        <select
                          value={tarifaSeleccionadaId}
                          onChange={(e) =>
                            setTarifaSeleccionadaId(Number(e.target.value))
                          }
                    ...
                    <div className="bg-white rounded-md border border-zinc-200 p-3 space-y-2">
                      <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">
                        Detalle del Comprobante ({cartItems.length} servicios)
                      </span>
                      {cartItems.length === 0 ? (
                        ...
                        <div className="space-y-1.5">
                          {cartItems.map((item) => (
                            <div
                              key={item.tarifaId}
                              className="flex items-center justify-between text-xs py-1 border-b border-zinc-100 last:border-none"
                            >
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    removeItemFromCart(item.tarifaId)
                                  }
                  ...
                  <div className="bg-zinc-50/50 rounded-lg p-4 border border-zinc-200/80 space-y-3">
                    <span className="text-[11px] font-semibold text-zinc-700 uppercase tracking-wider flex items-center gap-1.5">
                      <FileText className="h-3.5 w-3.5" /> 3. Comprobante SUNAT
                      y Forma de Pago
                    </span>
                    ...
                    {tipoComprobante === "FACTURA_ELECTRONICA" && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1 animate-in fade-in">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setRazonSocialFactura(e.target.value)
                          }
                    ...
                    <div className="grid grid-cols-4 gap-2 pt-1">
                      {[
                        ...
                        return (
                          ...
                          >
                            <Icon
                              className={`h-4 w-4 ${isSelected ? "text-zinc-50" : "text-zinc-500"}`}
                            />
                  ...
                  <div className="bg-zinc-950 text-zinc-50 rounded-lg p-4 space-y-2.5 shadow-sm border border-zinc-800">
                    <div className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider flex justify-between">
                      ...
                      <span>
                        Total Paciente: S/ {totalMontoPaciente.toFixed(2)}
                      </span>
                    </div>
                    <div className="grid grid-cols-4 gap-2 text-center text-xs pt-1">
                      <div className="bg-zinc-900 p-2 rounded-md border border-zinc-800">
                        ...
                        <div className="font-semibold text-zinc-50">
                          S/ {totalMontoPaciente.toFixed(2)}
                        </div>
                      </div>
                      <div className="bg-zinc-900 p-2 rounded-md border border-zinc-800">
                        <div className="text-[9px] text-zinc-400 uppercase font-medium">
                      ...
                      <div className="bg-zinc-900 p-2 rounded-md border border-zinc-800">
                        ...
                        <div className="font-semibold text-zinc-100">
                          S/{" "}
                          {(totalMontoTecnico + totalMontoSolicitante).toFixed(
                            2,
                          )}
                        </div>
                      </div>
          ...
          {activeTab === "importer" && currentUser?.rol === "ADMINISTRADOR" && (
            <ImporterPanel
              excelFile={excelFile}
              importing={importing}
          ...
          {activeTab === "alquileres" && <AlquileresPanel />}
      ...
      {showAdminModeModal && (
        <div className="fixed inset-0 bg-zinc-950/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4 select-none">
          <div className="bg-white border border-zinc-200 rounded-xl p-6 shadow-xl w-full max-w-sm flex flex-col gap-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[11px] text-zinc-500">
                Ingresa la contraseña de administrador para desbloquear las
                opciones avanzadas.
              </p>
            </div>
            ...
            >
              {adminModalError && (
                <div className="bg-red-50 text-red-700 border border-red-200 rounded p-2 text-[10px] flex items-center gap-1.5">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="h-9 w-full px-3 rounded border border-zinc-200 bg-white text-xs text-zinc-950 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                />
              </div>
              ...
              <div className="flex gap-2 justify-end mt-2">
                <button
                  type="button"
                  onClick={() => setShowAdminModeModal(false)}
                  disabled={adminModalLoading}
                  className="h-8 px-3 rounded border border-zinc-200 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 transition-all"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={adminModalLoading}
                  className="h-8 px-3 rounded bg-zinc-900 hover:bg-zinc-800 text-white text-xs font-semibold transition-all"
                >
                  {adminModalLoading ? "Verificando..." : "Confirmar"}
                </button>
              </div>
            </form>
          </div>
