      ...
      <div className="flex items-center justify-between border-b border-zinc-100 pb-4">
        <div>
          <h2 className="text-sm font-semibold text-zinc-900 flex items-center gap-2">
            <Activity className="h-4 w-4 text-zinc-900" /> Sala de Espera y Cola
            de Atención Médica
          </h2>
          <p className="text-xs text-zinc-555">
            Gestión de llamados a consultorio en tiempo real
          </p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {(["ESPERA", "CONSULTORIO", "ATENDIDO"] as EstadoAtencion[]).map(
          (est) => {
            const list = tickets.filter(
              (t) =>
                caja &&
                t.cajaDiariaId === caja.id &&
                (t.estadoAtencion || "ESPERA") === est,
            );
            const title =
              est === "ESPERA"
                ? "En Sala de Espera"
            ...
            return (
              <div
                key={est}
                className="bg-zinc-50/50 p-4 rounded-lg border border-zinc-200 space-y-3"
              >
                ...
                <div className="space-y-2">
                  {list.length === 0 ? (
                    ...
                    list.map((tk) => (
                      <div
                        key={tk.id}
                        className="bg-white p-3 rounded-md border border-zinc-200 space-y-2 shadow-sm"
                      >
                        ...
                        <div className="flex gap-1 pt-1">
                          {est === "ESPERA" && (
                            <button
                              onClick={() =>
                                onUpdateEstadoAtencion(tk.id, "CONSULTORIO")
                              }
                          ...
                          {est === "CONSULTORIO" && (
                            <button
                              onClick={() =>
                                onUpdateEstadoAtencion(tk.id, "ATENDIDO")
                              }
                              className="w-full bg-zinc-900 hover:bg-zinc-900/90 text-zinc-50 font-medium text-[10px] py-1 rounded transition"
                            >
                              Marcar Atendido
